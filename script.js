...
async function clicking(e) {
  ...
  var isRaining = out1.weather.some(
    (condition) => condition.main.toLowerCase() === "rain"
  );
  alert(`
    For ${parent.querySelector("h3").innerText}
    Latitude: ${lat}
    Longitude: ${lon}
    Current Humidity is ${out1.main.humidity}
    Current Pressure is ${out1.main.pressure}
    Current Temperature is ${out1.main.temp}K
    Is it raining? ${isRaining ? "Yes" : "No"}
  `);
  // console.log(out1);
